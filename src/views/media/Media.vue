            ...
            >
              <template #name="{item}">
                <td
                  ...
                  class="click-file"
                  @click="clickOnElement(item.id, item.type)"
                >
                  <CIcon :content="$options.folderIcon" />
                  {{ item.name }}
                </td>
              ...
              <template #actions="{item}">
                <td>
                  <CButton
                    color="primary"
                    @click="renameOpenForm(item.id, item.type)"
                  >
                    Rename
                  </CButton>
                </td>
                <td>
                  <CButton
                    color="primary"
                    @click="moveOpenForm(item.id, item.type, item.name)"
                  >
                    Move
                  </CButton>
                </td>
                ...
                <td>
                  <CButton
                    color="danger"
                    @click="openDeleteDialog(item.id, item.type, item.name)"
                  >
                    Delete
                  </CButton>
                </td>
...
export default {
  ...
  mounted() {
    ...
    document
      .getElementById('cropp-img-img')
      .addEventListener('load', this.updateCroppImage);
  },
  methods: {
    croppImage() {
      let self = this;
      self.cropper.getCroppedCanvas().toBlob(
        (blob) => {
    ...
    openCroppFileModal(id) {
      ...
      axios
        ...
        .then(function (response) {
          ...
          document
            .getElementById('cropp-img-img')
            .setAttribute('src', self.croppUrl);
          ...
          self.$nextTick(function () {
            if (self.cropper !== null) {
              ...
              self.cropper = new Cropper(
                document.getElementById('cropp-img-img'),
                {
