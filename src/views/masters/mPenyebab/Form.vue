        ...
        <CCardBody>
          <CForm method="POST" @submit.prevent="clickSubmitForm">
            <CRow class="mb-2 view-form">
              <CCol sm="12" md="6">
                <CInput
                  v-model="form.idPenyebab.val"
                  label="Kode Penyebab"
                  class="mb-2"
                  type="text"
                  placeholder="kode"
                  :readonly="mode == 'edit'"
                  :is-valid="form.idPenyebab.isValid"
                  @input="validateString(form.idPenyebab, { length: 2 })"
                />
                <p
                  v-if="form.idPenyebab.isValid == false"
              ...
              <CCol sm="12" md="6">
                <CInput
                  v-model="form.descPenyebab.val"
                  label="Deskripsi Penyebab"
                  class="mb-2"
                  type="text"
                  placeholder="deskripsi penyebab"
                  :is-valid="form.descPenyebab.isValid"
                  @input="validateString(form.descPenyebab, { length: 3 })"
                />
                <p
                  v-if="form.descPenyebab.isValid == false"
                  class="text-red-500 text-sm"
                >
                  *minimal 3 huruf
                </p>
...
export default {
  ...
  async mounted() {
    if (this.mode == 'create') {
      ...
      try {
        ...
        if (this.penyebab.length > 0) {
          const newId = await this.generateNewId();
          this.form.idPenyebab.val = newId;
        } else {
          this.form.idPenyebab.val = '01';
        }
      } catch (error) {
  ...
  methods: {
    async generateNewId() {
      const listPenyebab = this.penyebab.map((v) => {
        return parseInt(v.id);
      });
      return (Math.max(...listPenyebab) + 1).toString();
    },
    clickSubmitForm() {
      this.checkForm();
      this.loading = true;
      ...
      if (this.formIsValid) {
        this.$emit('click-submit-form', {
          ...
          mode: this.mode,
          data: {
            kode_penyebab: this.form.idPenyebab.val,
            diskripsi: this.form.descPenyebab.val,
          },
        });
      } else {
    ...
    checkForm() {
      const listValidate = [
        this.form.idPenyebab.isValid,
        this.form.descPenyebab.isValid,
      ];
