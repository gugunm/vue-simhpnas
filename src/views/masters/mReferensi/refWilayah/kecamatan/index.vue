    ...
    <CCard>
      <CCardBody>
        <CDataTable
          ...
          pagination
          clickable-rows
          @row-clicked="showDetailKecamatan"
        >
...
export default {
  ...
  computed: {
    items() {
      return this.refKecamatan ? this.refKecamatan.map((item, idx) => { return {...item, idx}}) : [];
    }
  },
  created () {
  ...
  methods: {
    ...
    showDetailKecamatan(item) {
      this.$router.push({ name: 'master-ref-wilayah-kecamatan', params: { idKecamatan: item.id, deskripsi: item.deskripsi } })
    },
