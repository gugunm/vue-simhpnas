  ...
  <CRow>
    <CCol sm="12">
      <div class="text-2xl mb-4 font-semibold">
        <h3 v-if="mode == 'create'">Create Jenis Obrik</h3>
        <h3 v-else>Edit Jenis Obrik</h3>
      </div>
      <CCard>
        <CCardBody>
          <CForm method="POST" @submit.prevent="clickSubmitForm">
            <CRow class="mb-2 view-form">
              <CCol sm="12" md="6">
                <CInput
                  v-model="form.idJenisObrik.val"
                  label="Kode Jenis Obrik"
                  class="mb-2"
                  type="text"
                  placeholder="kode"
                  :readonly="mode == 'edit'"
                  :is-valid="form.idJenisObrik.isValid"
                  @input="validateId(form.idJenisObrik, { length: 2 })"
                />
                <p
                  v-if="form.idJenisObrik.isValid == false"
              ...
              <CCol sm="12" md="6">
                <CInput
                  v-model="form.descJenisObrik.val"
                  label="Deskripsi Jenis Obrik"
                  class="mb-2"
                  type="text"
                  placeholder="deskripsi jenis obrik"
                  :is-valid="form.descJenisObrik.isValid"
                  @input="validateString(form.descJenisObrik, { length: 3 })"
                />
                <p
                  v-if="form.descJenisObrik.isValid == false"
                  class="text-red-500 text-sm"
                >
                  *minimal 3 huruf
                </p>
...
export default {
  ...
  methods: {
    async generateNewId() {
      const listJenisObrik = this.jenisObrik.map((v) => {
        return parseInt(v.id);
      });
      return (Math.max(...listJenisObrik) + 1).toString();
    },
    clickSubmitForm() {
      this.checkForm();
      this.loading = true;
      ...
      if (this.formIsValid) {
        this.$emit('click-submit-form', {
          ...
          mode: this.mode,
          data: {
            kode_jenis_obrik: this.form.idJenisObrik.val,
            diskripsi: this.form.descJenisObrik.val,
          },
        });
      } else {
    ...
    checkForm() {
      const listValidate = [
        this.form.idJenisObrik.isValid,
        this.form.descJenisObrik.isValid,
      ];
