  ...
  <CRow>
    <CCol sm="12">
      <div class="text-2xl mb-4 font-semibold">
        <h3 v-if="mode == 'create'">Create Group Lingkup Audit</h3>
        <h3 v-else>Edit Group Lingkup Audit</h3>
      </div>
      <CCard>
        <CCardBody>
            ...
            <CRow class="mb-2 view-form">
              <CCol sm="12" md="6">
                <CInput
                  ...
                  placeholder="kode"
                  :readonly="mode == 'edit'"
                  :is-valid="form.idGroupLingkupAudit.isValid"
                  @input="
                    validateString(form.idGroupLingkupAudit, { length: 1 })
                  "
                />
                <p
                  v-if="form.idGroupLingkupAudit.isValid == false"
              ...
              <CCol sm="12" md="6">
                ...
                />
                <p
                  v-if="form.descGroupLingkupAudit.isValid == false"
                  class="text-red-500 text-sm"
                >
                  *minimal 3 huruf
                </p>
...
export default {
    ...
    if (this.mode == 'create') {
      ...
      try {
        ...
        if (this.groupLingkupAudit.length > 0) {
          ...
          this.form.idGroupLingkupAudit.val = '01';
        }
      } catch (error) {
  ...
  methods: {
    async generateNewId() {
      const listGroupLingkupAudit = this.groupLingkupAudit.map((v) => {
        return parseInt(v.id);
      });
      return (Math.max(...listGroupLingkupAudit) + 1).toString();
    },
    clickSubmitForm() {
      this.checkForm();
      this.loading = true;
      ...
      if (this.formIsValid) {
        this.$emit('click-submit-form', {
          ...
          mode: this.mode,
          data: {
            kode_grup_lingkup_audit: this.form.idGroupLingkupAudit.val,
            diskripsi: this.form.descGroupLingkupAudit.val,
          },
        });
      } else {
    ...
    checkForm() {
      const listValidate = [
        this.form.idGroupLingkupAudit.isValid,
        this.form.descGroupLingkupAudit.isValid,
      ];
